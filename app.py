...
def generate_plots(N, mu, sigma2, S):
    ...
    for _ in range(S):
        X_sim = np.random.rand(N)  # Generate X values between 0 and 1
        Y_sim = mu + np.sqrt(sigma2) * np.random.randn(N)  # Generate Y with normal error
    ...
    plt.axvline(slope, color="blue", linestyle="--", linewidth=1, label=f"Initial Slope: {slope:.2f}")
    plt.axvline(intercept, color="orange", linestyle="--", linewidth=1, label=f"Initial Intercept: {intercept:.2f}")
